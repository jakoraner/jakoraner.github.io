window.onload = function init() {
    ...
    var currentShapePoints = []; // Points clicked in current mode
    ...
    var circleModeButton = document.getElementById('circle-mode-button');
    ...
    // Function to create circle vertices using triangles
    function createCircleVertices(center, radius, numSegments) {
        var circleVertices = [];
        ...
        for (var i = 0; i < numSegments; i++) {
            ...
            circleVertices.push(vec2(x1, y1));
            circleVertices.push(vec2(x2, y2));
        }
    ...
    canvas.addEventListener('click', function(event) {
        ...
        if (mode === 'point') {
            ...
            if (currentShapePoints.length === 1) {
                // First click: draw a small square (point)
                var size = 0.02; // Adjust as needed
                var pointVertices = createPointVertices(x, y, size);
                var pointColors = [];
                for (var i = 0; i < pointVertices.length; i++) {
                ...
                // Add to currentShape
                currentShape.vertices = pointVertices;
                currentShape.colors = pointColors;

            } else if (currentShapePoints.length === 2) {
                // Second click: draw the circle
                var center = currentShapePoints[0];
                var edgePoint = currentShapePoints[1];

                // Calculate radius
                var dx = edgePoint[0] - center[0];
                var dy = edgePoint[1] - center[1];
                var radius = Math.sqrt(dx * dx + dy * dy);

                // Create circle vertices
                var numSegments = 100; // Adjust for smoothness
                var circleVertices = createCircleVertices(center, radius, numSegments);
                var circleColors = [];
                for (var i = 0; i < circleVertices.length; i++) {
                    circleColors.push(pointColor);
                }

                // Replace the point with the circle
                currentShape.vertices = circleVertices;
                currentShape.colors = circleColors;
    ...
    // Event listener for circle mode button
