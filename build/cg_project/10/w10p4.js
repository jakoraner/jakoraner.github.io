window.onload = async function init() {
    ...
    var groundVertices = [
        vec4(-4.0, -1.0, 4.0, 1.0),  // Bottom-left
        vec4(4.0, -1.0, 4.0, 1.0),   // Bottom-right
        vec4(4.0, -1.0, -4.0, 1.0),  // Top-right
        vec4(-4.0, -1.0, -4.0, 1.0)  // Top-left
    ];
    ...
    var eye = vec3(0.0, 2.0, 5.0); // Eye position
    var at = vec3(0.0, 0.0, 0.0);  // Look-at point
    var up = vec3(0.0, 1.0, 0.0);  // Up vector

    var zeye = length(subtract(eye, at)); // Distance from eye to at point
    var xpan = 0.0;       // Pan along x-axis
    var ypan = 0.0;       // Pan along y-axis

    var projectionMatrix = perspective(60.0, canvas.width / canvas.height, 1.0, 100.0);

    // Variables for teapot motion
    var teapotY = 0.0; // Initial y position
    ...
    // Initialize quaternion for teapot rotation
    var q_teapot = new Quaternion();
    var q_inc = new Quaternion(); // Incremental rotation for spinning
    ...
    // Interaction mode: "orbit", "dolly", "pan", or "spinning"
    var interactionMode = "orbit"; // Default interaction mode

    // Add event listener for the dropdown menu
    var interactionSelect = document.getElementById("interaction-mode");
    interactionSelect.addEventListener("change", function () {
        interactionMode = interactionSelect.value;

        // Reset spinning state when changing modes
        if (interactionMode !== "spinning") {
            spinning = false;
            q_inc.setIdentity();
        }
    });
    ...
    function render() {
        ...
        if (spinning && (currentTime - lastMouseTime) < 20) {
            // Continue spinning
            q_teapot = q_inc.multiply(q_teapot);
        } else {
            // Stop spinning
            q_inc.setIdentity();
            spinning = false;
        ...
        var eyeDir = subtract(eye, at);
        eyeDir = normalize(eyeDir);
        var eyePosition = add(scale(zeye, eyeDir), at);

        // Apply panning
        var upVector = up;
        var rightVector = cross(eyeDir, upVector);

        var panOffset = add(scale(xpan, rightVector), scale(ypan, upVector));
        eyePosition = add(eyePosition, panOffset);
        at = add(at, panOffset);

        // Compute view matrix
        var viewMatrix = lookAt(eyePosition, at, upVector);
        ...
        // Apply teapot rotation
        var rotationMatrix = q_teapot.get_mat4();
        modelMatrixTeapotCurrent = mult(modelMatrixTeapotCurrent, rotationMatrix);

        // Apply translation
        modelMatrixTeapotCurrent = mult(modelMatrixTeapotCurrent, translate(0, teapotY, 0));
    ...
    function initEventHandlers(canvas) {
        ...
        var lastPos = null; // Last position on the virtual trackball
        ...
        canvas.onmousedown = function(ev) { // Mouse is pressed
            ...
            if (interactionMode === "orbit" || interactionMode === "spinning") {
                // Convert coordinates to virtual trackball vector
                var pos = getTrackballVector(x, y, canvas.width, canvas.height);
                lastPos = pos;
        ...
        canvas.onmouseup = function (ev) {
            ...
            if (interactionMode === "spinning") {
                ...
                var x = ev.clientX - rect.left;
                var y = ev.clientY - rect.top;
                var currentPos = getTrackballVector(x, y, canvas.width, canvas.height);

                // Check if mouse was released at the same position
                var samePosition = (length(subtract(currentPos, lastPos)) < 0.0001);

                // Stop spinning if the last mouse position is the same or if time elapsed
                if (samePosition || (Date.now() - lastMouseTime) > 20) {
                    spinning = false;
                    q_inc.setIdentity();
                } else {
                    spinning = true;
            ...
            lastX = -1; // Reset last mouse position
            lastY = -1;
        };

        canvas.onmousemove = function (ev) { // Mouse is moved
            if (!dragging) return; // Only act if dragging
            ...
            if (interactionMode === "orbit" || interactionMode === "spinning") {
                ...
                // Calculate the quaternion representing the rotation from lastPos to currentPos
                q_inc = new Quaternion();
                q_inc = q_inc.make_rot_vec2vec(lastPos, currentPos);

                // Update the teapot's rotation quaternion
                q_teapot = q_inc.multiply(q_teapot);

                lastPos = currentPos;
                lastMouseTime = Date.now();
                spinning = (interactionMode === "spinning");
            } else if (interactionMode === "dolly") {
                if (lastY >= 0) { // Prevent invalid initial motion
                    var dy = y - lastY;

                    // Update zeye based on dy
                    var factor = 0.05; // Adjust sensitivity
                    zeye += factor * dy;

                    // Limit zeye to prevent going through the origin
                    zeye = Math.max(1.0, Math.min(zeye, 100.0));
                }
                lastY = y;
            } else if (interactionMode === "pan") {
                if (lastX >= 0 && lastY >= 0) { // Prevent invalid initial motion
                    var dx = x - lastX;
                    var dy = y - lastY;

                    var factor = 0.005; // Adjust sensitivity

                    // Transform world space basis vectors using current rotation
                    var right = q_teapot.apply(vec3(1, 0, 0)); // x-axis
                    var upVec = q_teapot.apply(vec3(0, 1, 0)); // y-axis

                    // Compute the panning offset
                    var offset = add(scale(-factor * dx, right), scale(factor * dy, upVec));
    ...
    // Helper function to map mouse position to virtual trackball vector
    function getTrackballVector(x, y, width, height) {
        // Map x and y to [-1, 1]
        var p_x = (x - width / 2) / (width / 2);
        var p_y = (height / 2 - y) / (height / 2);

        var d = p_x * p_x + p_y * p_y;

        var v;

        var v0 = 1 / Math.SQRT2; // 1 / sqrt(2)
