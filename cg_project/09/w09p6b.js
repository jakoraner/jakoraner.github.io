...
var texelSize = vec2(1.0 / OFFSCREEN_WIDTH, 1.0 / OFFSCREEN_HEIGHT);

    // Get uniform locations for texel size
    var u_TexelSizeGround = gl.getUniformLocation(programGround, 'u_TexelSize');
    var u_TexelSizeTeapot = gl.getUniformLocation(programTeapot, 'u_TexelSize');

    // Set the texel size uniform before rendering
    function setTexelSize() {
        gl.useProgram(programGround);
        gl.uniform2fv(u_TexelSizeGround, flatten(texelSize));

        gl.useProgram(programTeapot);
        gl.uniform2fv(u_TexelSizeTeapot, flatten(texelSize));
    }
    ...
    var viewMatrix = lookAt(
        vec3(0.0, 3.0, 5.0),  // Eye position closer to the ground
        vec3(0.0, -1.0, -5.0), // Look-at point further along the z-axis
        vec3(0.0, 1.0, 0.0)    // Up vector remains the same
    );

    var projectionMatrix = perspective(23.0, canvas.width / canvas.height, 1.0, 100.0);
